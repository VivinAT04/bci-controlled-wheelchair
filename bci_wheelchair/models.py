...
from .data_loading import SFREQ

DEFAULT_BANDS = [(4, 8), (8, 12), (12, 16), (16, 20),
...
class FilterBankCSP(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for lo, hi in self.bands:
            ...
            csp.fit(bandpass(X, lo, hi, self.sfreq), y)
            self.csps_.append(csp)
        return self

    def transform(self, X):
        feats = [csp.transform(bandpass(X, lo, hi, self.sfreq))
...
def make_eegnet():
    """Stub: add an EEGNet (Lawhern et al., 2018) model here in Phase 3."""
