...
OUTPUT_PATH = Path(
    "results/cross_subject/csp_fbcsp/cross_subject_a09_ea_csp10_subject_zscore_predictions.csv"
)

SUMMARY_PATH = Path(
    "results/cross_subject/csp_fbcsp/cross_subject_a09_ea_csp10_subject_zscore_summary.csv"
)
...
def subjectwise_channel_standardize(
    X: np.ndarray,
    epsilon: float = 1e-8,
) -> np.ndarray:
    """
    Standardize one subject independently, channel by channel.

    The mean and standard deviation for each EEG channel are
    calculated across all trials and all time samples belonging
    to that subject.
    ...
    channel_standard_deviation = np.std(
        X,
        axis=(0, 2),
        keepdims=True,
    )

    channel_standard_deviation = np.maximum(
...
def load_and_align_subject(
    ...
    X_aligned = subjectwise_channel_standardize(
        X_aligned
    )
    ...
    standardized_standard_deviation = float(
        np.std(X_aligned)
    )

    print(
        "  Standardized global mean: "
        f"{standardized_mean:.6f}"
    )

    print(
        "  Standardized global std:  "
        f"{standardized_standard_deviation:.6f}"
    )
...
def main() -> None:
    ...
    print("\n" + "=" * 76)
    print(
        "EA CSP-10 + Subject-wise Z-score Results"
    )
    print("=" * 76)
