"""
Run Euclidean Alignment + EEGNet LOSO evaluation.

Each subject is aligned independently before training.

Example fold:

    Training:
        Euclidean Alignment on A01T
        Euclidean Alignment on A02T
        ...
        Euclidean Alignment on A08T

    Testing:
        Euclidean Alignment on A09T

The held-out subject labels are never used for:
...
Run one fold:
    ...
    python -m scripts.eegnet.run_eegnet_ea_loso \
        --test-subject A09
...
from bci_wheelchair.data.alignment import (
    euclidean_align,
)
...
class ChannelStandardizer:
    ...
    def transform(
        ...
        """Apply fitted channel normalisation."""
        X = self._validate(X)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Euclidean Alignment + "
            "EEGNet LOSO evaluation."
        )
    )
...
def load_subject(
    ...
    if X.ndim != 3:
        raise ValueError(
            f"Invalid EEG shape in {path}: "
            f"{X.shape}"
        )

    if len(X) != len(y):
        raise ValueError(
            "Trial and label counts differ "
            f"in {path}."
        )
...
def load_and_align_subject(
    ...
    """
    Load and independently align one subject.

    Labels are not used during alignment.
    """
    X, y = load_subject(
        data_directory,
        subject,
...
def combine_aligned_subjects(
    data_directory: Path,
    subjects: list[str],
) -> tuple[np.ndarray, np.ndarray]:
    """
    Align each training subject independently and combine them.
    """
...
def train_model(
    ...
    """Train EEGNet on aligned training subjects."""
    set_random_seed(
...
def predict(
    ...
    """Predict labels for aligned unseen EEG."""
    device = next(
        model.parameters()
    ).device
...
def run_fold(
    ...
    print(
        "Method: Euclidean Alignment "
        "+ EEGNet"
    )
    print("=" * 76)

    X_train, y_train_text = (
        combine_aligned_subjects(
            args.data_directory,
            training_subjects,
        )
    )
    ...
    print(
        f"Aligned training EEG: "
        f"{X_train.shape}"
    )
    print(
        f"Aligned held-out EEG: "
        f"{X_test.shape}"
    )
    print(
...
def create_summary(
    ...
    """Summarise EA + EEGNet performance."""
...
def main() -> None:
    """Run one or all EA + EEGNet folds."""
